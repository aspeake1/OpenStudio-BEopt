...
# start the measure
class ResidentialHotWaterHeaterHeatPumpApproximation < OpenStudio::Measure::ModelMeasure
  ...
  def description
    return "This measure adds an approximation of a new residential heat pump water heater to the model based on user inputs. If there is already an existing residential water heater in the model, it is replaced. For multifamily buildings, the water heater can be set for all units of the building.#{Constants.WorkflowDescription}"
  end

  def modeler_description
    return "The measure will create a new instance of the OS:WaterHeater:Mixed object representing an approximation of a heat pump water heater and EMS code for the controls. The water heater will be placed on the plant loop 'Domestic Hot Water Loop'. If this loop already exists, any water heater on that loop will be removed and replaced with a water heater consistent with this measure. If it doesn't exist, it will be created."
  end
  ...
  def run(model, runner, user_arguments)
    ...
    units.each_with_index do |unit, unit_index|
      ...
      success = Waterheater.apply_heatpump_approximation(model, unit, runner, space, weather,
...
ResidentialHotWaterHeaterHeatPumpApproximation.new.registerWithApplication
