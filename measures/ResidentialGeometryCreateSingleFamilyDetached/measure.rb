...
class CreateResidentialSingleFamilyDetachedGeometry < OpenStudio::Measure::ModelMeasure
  ...
  def description
    return "Sets the basic geometry for the building. Building is limited to one foundation type. Garage is tucked within the building, on the front left or front right corners of the building. Sets the number of bedrooms and bathrooms in the building.#{Constants.WorkflowDescription}"
  end

  # human readable description of modeling approach
  def modeler_description
    return "Gathers living space area, wall height per floor, number of floors, aspect ratio, garage width and depth, garage position, foundation type and wall height, attic and roof type, and roof pitch. Constructs building by calculating footprint and performing a series of affine transformations into living, foundation, and attic spaces. Also, sets (or replaces) BuildingUnit objects that store the number of bedrooms and bathrooms associated with the model."
  end
  ...
  def run(model, runner, user_arguments)
    ...
    if roof_type != Constants.RoofTypeFlat
      ...
      if attic_type == "unfinished attic"
        ...
        attic_space_type_name = Constants.SpaceTypeUnfinishedAttic
      elsif attic_type == "finished attic"
